...
def _print_eval(entry: dict, eval_splits: list[str]) -> None:
    parts = [f"step {entry['step']:>6}  epoch {entry['epoch']:.2f}  "
             f"elapsed {entry['elapsed_s']:.0f}s"]
    for split in eval_splits:
        acc_key     = f"{split}_acc"
        bal_acc_key = f"{split}_balanced_acc"
        f1_key      = f"{split}_f1"
        if acc_key in entry:
            s = f"{split}: acc={entry[acc_key]:.3f}"
            if bal_acc_key in entry:
                s += f" bal={entry[bal_acc_key]:.3f}"
            if f1_key in entry:
                s += f" f1={entry[f1_key]:.3f}"
            parts.append(s)
    print("  ".join(parts))


def _print_epoch(epoch: int, total: int, loss: float, acc: float) -> None:
    print(f"Epoch {epoch + 1}/{total}  train_loss={loss:.4f}  train_acc={acc:.3f}")
